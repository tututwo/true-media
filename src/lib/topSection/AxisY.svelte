...
<script>
  ...
  const { data, xRange, yScale,config } = getContext("LayerCake");
...
<div class="axis y-axis">
  {#each tickVals as tick, i (tick)}
    ...
    {@const dataPoint = $data.find((d) => d[$config.y] === tick).topN[0]}

    <div
      class="tick tick-{i}"
      style="left:{$xRange[0]}%;top:{tickValPerc + halfBand}%;"
    >
      {#if gridlines === true}
        <div
          class="gridline"
      ...
      {/if}
      <div
        class="text flex items-center absolute"
        style:top="0"
      ...
      >
        <img
          class="w-[25px] h-[21px] pr-2"
          src={dataPoint.teamImage}
          alt={dataPoint.currentTeamName}
        />
        <h6 class="whitespace-nowrap font-semibold text-[14px]">{dataPoint.currentTeamName}</h6>
      </div>
    </div>
  {/each}
</div>
